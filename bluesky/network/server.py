...
class Server(Thread):
    ...
    def delnode(self, node_id):
        ''' Terminate and remove a single simulation node from this server.
            ...
            last node is allowed: the server stays up with zero nodes, and
            new nodes can still be spawned with ADDNODES.
        '''
        if isinstance(node_id, str):
            try:
                node_id = hex2bin(node_id)
            except ValueError:
                print('DELNODE: invalid node id', node_id)
                return
        ...
        if proc is None:
            print('DELNODE: unknown node id', bin2hex(node_id))
            return
