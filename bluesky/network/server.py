...
from bluesky.network.common import genid, bin2hex, hex2bin, MSG_SUBSCRIBE, MSG_UNSUBSCRIBE, GROUPID_SIM, IDLEN
...
class Server(Thread):
    ...
    def echo(self, text, dest=None):
        ''' Log a message, and echo it to a connected client if dest is given. '''
        print(text)
        if dest:
            self.send(b'ECHO', dict(text=text, flags=0), dest)

    def delnode(self, node_id, sender_id=None):
        ''' Terminate and remove a single simulation node from this server.

            Arguments:
            - node_id: The id of the node to delete. Either a bytestring,
              or its hexadecimal string representation (as generated by
              common.bin2hex).
            - sender_id: When given, rejections are echoed back to this
              connection in addition to the server log.

            An unknown node id is ignored (logged, no error). Deleting the
            last remaining node is refused: use QUIT to stop the server,
            which also stops its last node.
        '''
        if isinstance(node_id, str):
            try:
                node_id = hex2bin(node_id)
            except ValueError:
                self.echo(f'DELNODE: invalid node id {node_id}', sender_id)
                return
        if not isinstance(node_id, bytes):
            self.echo(f'DELNODE: invalid node id {node_id}', sender_id)
            return
        proc = self.spawned_processes.get(node_id)
        if proc is None:
            self.echo(f'DELNODE: unknown node id {bin2hex(node_id)}', sender_id)
            return
        if len(self.spawned_processes) == 1:
            self.echo('DELNODE: cannot delete the last node. '
                      'Use QUIT to stop the server instead.', sender_id)
            return
        print('Stopping node', bin2hex(node_id))
        # Signal only this child. On Windows each node is spawned in its own
        # process group (CREATE_NEW_PROCESS_GROUP), so CTRL_BREAK_EVENT only
        # reaches this node. Elsewhere terminate() signals a single pid.
        if platform.system() == 'Windows':
        ...
        try:
            ...
            proc.kill()
            proc.wait()
        # Remove all bookkeeping for this node
        self.spawned_processes.pop(node_id, None)
        self.sim_nodes.discard(node_id)
        self.avail_nodes.discard(node_id)
        # Inform the network that the node is removed
        self.sock_recv.send_multipart([b'\x00' + node_id])
                    ...
                    if msg[0].startswith(self.server_id):
                        ...
                        if topic == b'QUIT':
                            ...
                            if isinstance(data, int):
                                ...
                                self.addnodes(**data)
                        elif topic == b'DELNODE':
                            self.delnode(data, sender_id)
